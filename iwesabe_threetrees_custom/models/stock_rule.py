...
class StockRule(models.Model):
    ...
    def _run_buy(self, procurements):
        """ OVERIDE: after creating RFQ send mail to purchase user"""
        ...
        for domain, procurements_rules in procurements_by_po_domain.items():
            ...
            purchase = self.env['purchase.order.line'].sudo().create(po_line_values)
            if purchase:
                purchase_user_group = self.env.ref('purchase.group_purchase_user')
                base_user = self.env.ref('base.group_user')
                purchase_users = self.env['res.users'].sudo().search([('groups_id', 'in', [purchase_user_group.id, base_user.id])])
                if purchase_users:
                    template = self.env.ref('iwesabe_threetrees_custom.mail_template_rfq_created', raise_if_not_found=False)
                    template.sudo().send_mail(po.id, force_send=True, email_values={'email_to': ','.join(user.email for user in purchase_users if user.email) or self.env.user.email})
